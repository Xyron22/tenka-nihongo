...
function examSettingsHtml(){
  ...
  <div class="muted-box">Feedback jawaban memakai MP3 pilihan kamu. Voice Jepang tetap hanya untuk mulai, selesai, dan perfect.</div>
  ...
  <div class="row"><div style="flex:1"><b>🔉 Volume</b><small>${Math.round((s.volume??.82)*100)}%</small></div><input aria-label="Volume" type="range" min="0" max="1" step="0.05" value="${s.volume??.82}" onchange="setAudioSetting('volume',this.value)" style="width:145px"></div>
  ...
  <div class="section-title">🎚️ Test SFX pilihan kamu</div>
  <div class="small-actions"><button class="pill" onclick="previewAudio('correct')">✅ Ding</button><button class="pill" onclick="previewAudio('wrong')">❌ Error</button><button class="pill" onclick="previewAudio('combo')">🔥 Combo</button><button class="pill" onclick="previewAudio('timeout')">⏱️ BRUH</button></div>
...
function refresh(){
  ...
  if(!box||box.dataset.examSoundV5==='1')return;
  box.dataset.examSoundV5='1';box.innerHTML=examSettingsHtml();
...
window.TENKA_AUDIO_UI={version:'5.0.0',refresh};
