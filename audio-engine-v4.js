...
const EXAM_DEFS={
  correct:[[880,.060,.34,0],[1175,.085,.30,0]],
  wrong:[[220,.095,.34,0],[175,.115,.30,0]],
  combo:[[659,.050,.25,0],[880,.060,.28,0],[1175,.090,.31,0]],
  timeout:[[440,.085,.29,.025],[330,.115,.29,0]],
  click:[[700,.045,.18,0]]
};
...
function wavUrl(tones){
  ...
  for(const [freq,dur,amp,gap] of tones){
    const n=Math.max(1,Math.floor(rate*dur));
    for(let i=0;i<n;i++){
      const t=i/rate,fade=Math.min(1,i/(rate*.008+1),(n-i)/(rate*.018+1));
      const v=Math.sin(2*Math.PI*freq*t)*amp*fade;
      samples.push(Math.max(-32767,Math.min(32767,Math.round(v*32767))));
    }
    for(let i=0;i<Math.floor(rate*gap);i++)samples.push(0);
  }
  const buf=new ArrayBuffer(44+samples.length*2),v=new DataView(buf),bytes=samples.length*2;
  writeAscii(v,0,'RIFF');v.setUint32(4,36+bytes,true);writeAscii(v,8,'WAVE');writeAscii(v,12,'fmt ');
  ...
  writeAscii(v,36,'data');v.setUint32(40,bytes,true);for(let i=0;i<samples.length;i++)v.setInt16(44+i*2,samples[i],true);
  return URL.createObjectURL(new Blob([buf],{type:'audio/wav'}));
}
function buildExamPlayers(){
  for(const [event,tones] of Object.entries(EXAM_DEFS)){
    try{
      const url=wavUrl(tones),a=new Audio(url);a.preload='auto';a.volume=settings.volume;examPlayers[event]={audio:a,url};
...
function playEvent(event){
  if(!settings.enabled||!EVENTS.includes(event))return false;stopAll();const token=playToken;
  if(EXAM_EVENTS.has(event)){
    const ok=playExam(event);eventLog.push({event,source:'exam-wav',ok,at:Date.now()});if(eventLog.length>100)eventLog.shift();return ok;
  }
...
function debug(){return{version:VERSION,settings:getSettings(),primeAttempted,lastExamError,examPlayers:Object.keys(examPlayers),counts:Object.fromEntries(SOURCES.map(s=>[s,sourceCount(s)])),events:Object.fromEntries(EVENTS.map(e=>[e,SOURCES.reduce((n,s)=>n+eventCount(s,e),0)])),eventLog:eventLog.slice(-20),activeExam:!!activeExam,activeVoice:!!activeVoice}}
