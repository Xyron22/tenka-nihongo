...
assert(context.TENKA_AUDIO_VERSION==='5.0.0','version');
...
assert(d0.examFiles.combo.includes('assets/audio/exam/correct.mp3'),'combo should reuse approved ding until custom combo chosen');
...
n=plays('correct.mp3');context.TENKA_AUDIO.playEvent('combo');assert(plays('correct.mp3')===n+1,'combo must reuse exactly one ding MP3');assert(spoken.length===0,'combo must never speak');
...
assert(!/AudioContext|webkitAudioContext/.test(src),'Exam Sound v5 must not use Web Audio');
const d=context.TENKA_AUDIO.debug();assert(d.eventLog.slice(-4).every(x=>x.source==='myinstants-mp3'),'answer events must be logged as MyInstants MP3');
console.log('TENKA MyInstants Exam Sound v5 tests passed');
