...
(async()=>{
  ...
  }
  {
    ...
    assert(b.html.includes('<ruby')&&b.html.includes('<rt>'),'Houkoku puzzle must render furigana above Japanese text');
    ...
    assert(b.html.includes('🇮🇩')&&b.html.includes('Saat dilihat, beliau sudah duduk di lantai.'),'Houkoku result must show Indonesian meaning for each sentence');
    assert(b.html.includes('<ruby')&&b.html.includes('<rt>'),'Houkoku result must preserve furigana');
